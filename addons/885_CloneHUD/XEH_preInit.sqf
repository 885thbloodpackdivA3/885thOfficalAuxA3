...
// --- Squad Radar (adapted from Squad Radar Health reference PBO) ---
[
    "BPD_HUD_radarEnabled",
    "CHECKBOX",
    ["Enable Squad Radar", "Shows the circular squad radar and status panel"],
    ["885th Clone HUD", "Squad Radar"],
    true,
    false,
    {
        params ["_value"];
        if (!hasInterface) exitWith {};
        if (_value) then {
            if (!isNull player) then { call BPD_HUD_fnc_createRadar; };
        } else {
            call BPD_HUD_fnc_destroyRadar;
        };
    }
...
[
    "BPD_HUD_radarSize",
    "SLIDER",
    ["Radar Size", "Diameter of the squad radar as a fraction of screen height"],
    ["885th Clone HUD", "Squad Radar"],
    [0.08, 0.3, 0.16, 2],
    false
] call CBA_fnc_addSetting;
...
[
    "BPD_HUD_radarRange",
    "SLIDER",
    ["Radar Range (m)", "Starting radar range - can also be zoomed with Page Up/Page Down in-game"],
    ["885th Clone HUD", "Squad Radar"],
    [20, 500, 200, 0],
    false
] call CBA_fnc_addSetting;
...
[
    "BPD_HUD_radarShowNames",
    "CHECKBOX",
    ["Show Names on Radar", "Shows callsigns next to squad contact dots"],
    ["885th Clone HUD", "Squad Radar"],
    true,
    false
] call CBA_fnc_addSetting;
...
[
    "BPD_HUD_radarShowDistance",
    "CHECKBOX",
    ["Show Distance on Radar", "Shows distance next to squad contact dots"],
    ["885th Clone HUD", "Squad Radar"],
    true,
    false
] call CBA_fnc_addSetting;
...
[
    "BPD_HUD_radarPanelEnabled",
    "CHECKBOX",
    ["Enable Squad Status Panel", "Shows the list panel with names, distance, and health bars"],
    ["885th Clone HUD", "Squad Radar"],
    true,
    false
] call CBA_fnc_addSetting;
